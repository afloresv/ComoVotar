...
$(document).ready(function(){
	...
	var opciones_show = "";
	for (var i=1 ; i<=27 ; i++)
		opciones_show += "<div class='color"+(i%11)+"'>P"+i+"</div>";
	...
	$('#MultipleLink #selec_todo_link').click(function(){
		...
		showNadie.hide();
		$('#showPartido > div').slideDown(300);
	});

	MultipleLinkEach.click(function(){
		var to = $(this).attr('to'),
			votoOpt = $('#MultipleImg .voto_led_'+to);
		if (votoOpt.hasClass('active')) voto--;
		else voto++;

		if (voto==0) setTimeout(function(){showNadie.show();},310);
		else showNadie.hide();

		votoOpt.toggleClass('active');
		$('#showPartido > .show_voto_'+to).slideToggle(300);
	});
	...
	$('#ModalBack > div span.close').click(function(){
		...
		var windowModal = $(this).parent().parent();
		windowModal.hide();
		if (windowModal.attr('noHandReturn')==null)
			lockMouse = false;
	});

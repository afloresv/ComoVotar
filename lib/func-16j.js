...
$(document).ready(function(){
	...
	// Days left
	var today = new Date(),
		election = new Date("Jul 16 2017 07:00:00 GMT-0500");
	var faltan = Math.ceil((election.getTime()-today.getTime())/3600000);
	if (faltan < 0) {
		$('#LlegoElDia').show();
	} else if (faltan < 72) {
		$('#CuantoFalta > span:first-child').text(faltan);
		$('#CuantoFalta').attr('class','FaltanHoras').show();
	} else {
		$('#CuantoFalta > span:first-child').text(Math.ceil(faltan/24));
		$('#CuantoFalta').attr('class','FaltanDias').show();
	}
